...
int main()
{
	...
	FILE *fp;
	const char fileName[] = "./pi_kernel.cl";
	size_t source_size;
	char *source_str;
	cl_int i;
	...
	Sum = (double *)malloc(ARRSIZE*sizeof(double));
	...
	//Allocate memory
	Sumd = clCreateBuffer(context, CL_MEM_WRITE_ONLY, ARRSIZE * sizeof(double), NULL, &ret);
	...
	ret = clSetKernelArg(kernel, 1, sizeof(double), &N);
	...
	// Copy results back
	ret = clEnqueueReadBuffer(command_queue, Sumd, CL_TRUE, 0, ARRSIZE*sizeof(double), Sum, 0, NULL, NULL);
