...
int main()
{
	...
	int N = ARRSIZE;
	FILE *fp;
	const char fileName[] = "./mat_mat_kernel.cl";
	size_t source_size;
	char *source_str;
	cl_int i;
	...
	B = (int *)malloc(ARRSIZE*ARRSIZE*sizeof(int));
	C = (int *)malloc(ARRSIZE*ARRSIZE*sizeof(int));
	...
	Bd = clCreateBuffer(context, CL_MEM_READ_ONLY, ARRSIZE * ARRSIZE * sizeof(int), NULL, &ret);
	Cd = clCreateBuffer(context, CL_MEM_WRITE_ONLY, ARRSIZE * ARRSIZE * sizeof(int), NULL, &ret);

	
	// Copy data to allocated memory
	ret = clEnqueueWriteBuffer(command_queue, Ad, CL_TRUE, 0, ARRSIZE * ARRSIZE * sizeof(int), A, 0, NULL, NULL);
	ret = clEnqueueWriteBuffer(command_queue, Bd, CL_TRUE, 0, ARRSIZE * ARRSIZE * sizeof(int), B, 0, NULL, NULL);
	...
	// Create Kernel
	kernel = clCreateKernel(program, "mat_mat_mul", &ret);
	...
	size_t global_work_size = ARRSIZE*ARRSIZE;
	...
	// Copy results back
	ret = clEnqueueReadBuffer(command_queue, Cd, CL_TRUE, 0, ARRSIZE*ARRSIZE*sizeof(int), C, 0, NULL, NULL);
